...
const MyTabs = () => {
  return (
      ...
      screenOptions={({ route }) => ({
        tabBarIcon: ({ focused, color, size }) => {
          ...
          if (route.name === 'Tin nhắn') {
            iconName = focused ? 'chatbubble-ellipses-outline' : 'chatbubble-outline';
        ...
        tabBarLabel: ({ focused }) => {
          ...
          return (
            <Text style={{ color: focused ? '#4E7DFF' : '#8E8E93', fontSize: 12 }}>
              {label}
            </Text>
          );
        },
    ...
    >
      <Tab.Screen name="Tin nhắn" component={Home} options={{ headerShown: false }} />
      <Tab.Screen name="Danh bạ" component={Contacts} options={{ headerShown: false }} />
      <Tab.Screen name="Nhật ký" component={Activity} options={{ headerShown: false }} />
