...
const GroupInfo = () => {
  ...
  const currentUserMember = groupMembers.find(m => m.user_id === user._id);
  ...
  const copyInviteLink = async () => {
    try {
      ...
      await Clipboard.setStringAsync(inviteLink); // Updated Clipboard API
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      const filteredResults = results
        .filter(result => result?._id && !groupMembers.some(member => member.user_id === result._id))
        .map(result => ({
          ...result,
            ...
            { text: 'Hủy', style: 'cancel' },
            {
              ...
              onPress: async () => {
                try {
                  ...
                  setMembers(groupMembers.filter(member => member.user_id !== memberId));
